...
class PoseEstimator:
    def __init__(self, model_path):
        ...
        self.inp_len = self.interpreter.get_input_details()[0]['shape'][1]
    ...
    def predict(self, frame):
        ...
        for i in range(pred.shape[0]):
            ...
            raw_y = xy[1]*self.out_len  # 找到最大位置所在行-->   x坐标值
...
def convert(img_dirs, onnx_path):
    ...
    img_dirs = glob.glob(img_dirs + '/*')
    for img_dir in img_dirs:
        if os.path.isdir(img_dir):
            names = os.listdir(img_dir + '/images')
            names.sort()
            for name in names:
                file = os.path.join(img_dir, 'images', name)
                img = cv2.imread(file)
                if img is None:
                    continue
                res = p.predict(img)
                ...
                imgPath = os.path.join(img_dir, 'images', person["image"])
                print(imgPath)
            ...
            with open(os.path.join(img_dir, "pose-data_blaze.csv"), 'w', newline='') as csvfile:
                csvwriter = csv.writer(csvfile)
                csvwriter.writerow(headers)
                csvwriter.writerows(csv_output_rows)
                csvfile.close()
            # csv_output_rows = list()
